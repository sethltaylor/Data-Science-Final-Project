...
library(stargazer)
stargazer(model.data, title="Descriptive Statistics", digits=2, header = FALSE)

```

```{r echo = FALSE, warning = FALSE, results = 'asis'}
factorvar <- data.frame(model.data$EDUCATION, model.data$AGE_CAT, model.data$TYPEOFFENSE)
stargazer(summary(factorvar), title = 'Descriptive Statistics: Factor Variables', header = FALSE)
...
c.mat.glm <- table(test.real, test.recid)

accuracy.glm <- (c.mat.glm[1,1] + c.mat.glm[2,2])/nrow(trainset) # (TP + TN) / N

tpr.glm <- c.mat.glm[2,2] / (c.mat.glm[2,2] + c.mat.glm[2,1]) # TP/(TP+FN)

tnr.glm <- c.mat.glm[1,1] / (c.mat.glm[1,1] + c.mat.glm[1,2]) # TN/(TN+FP)

ppv.glm <- c.mat.glm[2,2] / (c.mat.glm[2,2] + c.mat.glm[1,2]) # TP/(TP+FP)

glm.acc.meas <- data.frame()


acc.meas <- c(accuracy.glm, tpr.glm, tnr.glm, ppv.glm)

glm.acc.meas <- data.frame(acc.meas)
row.names(glm.acc.meas) <- c("Accuracy", "True Positive Rate", "True Negative Rate", "Positive Predictive Value")
colnames(glm.acc.meas) <- c("GLM Accuracy Measures")
  
kable(glm.acc.meas, format = 'markdown')
```
...
mfx.fit <- logitmfx(CH_CRIMHIST_COLLAPSED ~ OFFENSE_VIOLENT + OFFENSE_DRUG + OFFENSE_PROPERTY + CS_SENTENCEMTH +  
           + SES_PARENTS_INCARCERATED + SES_FAMILY_INCARCERATED + SES_HASCHILDREN + AGE_CAT + 
           + SES_SEXABUSED_EVER + DRUG_ANYREG + state + GENDER + DRUG_COCRKTME + DRUG_HROPTME + DRUG_ANYTME + DRUG_METHATME +
           + CH_PRIORARREST_CAT + TYPEOFFENSE + DRUG_TRT, data = full.numeric, robust = TRUE)

stargazer(mfx.fit$mfxest, title = 'Logistic Regression: Marginal Effects', header = FALSE)
```

Our model shows that the primary factors that influence recidivism are drug use, the type of offense committed, prior criminal history, and family criminal history. Education and length of setnence are both not signfiicant. Prisoners who committed a violent crime are 7 percentage points less likey to be a recidivist. If the offense was a property crime, then the probability rises by about 9 percent. The type of offense variable is in comparison to murders, and shows that individuals who commit kidnapping, sexual assault, robbery, assualt, or other violent crimes are between 3 and 8 percentage points more likely to be recidivists than prisoners who committed murder. Individuals sentenced for drug possession, weapon crimes, DWIs, and other public order offenses are also between 4 and 8.7 percentage points more likley to be recidivists when compared against murderers. Finally, for every prior arrest an individual has, their probability of being a recidivist increases by 4.7 percentage points. 


If an individuals parents were incarcerated, then they are about 1.9 percentage points more likely to be a recidivist, and if anyone in their family was incarcerated they are about 2.7 percentage points more likely to be a recidivist. 


Age is consistently a positive factor for recidivism, except for the highest age category. This is likely due to the fact that younger prisoners have more time to re-enter the system. People between the ages of 65-96 are less likely to be recommitting offenses. 


Drug use is also a prominent factor in recidivism. Individuals who regularly did drugs were about 5.7 percent more likely to be recidivists. While individuals who were using any drug at the time of arrest are about 2.3 percentage points less likely to be recidivists, individuals who were using heroin, crack, or methamphetamines are between 3 and 4 percentage points more likley to be recidivists. This suggests that harder drug use is more common among recidivists while drug use like marijuana is more common among first offenders. Individuals who had been in drug treatment programs are about 4 percentage points more likely to be recidivists. This is likely to be a reverse causality in that recidivists are more often hard drug users and therefore more likely to have ever gone to drug treatment programs, since it is unclear why drug treatment programs would make you more likely to commit a crime. 


Finally, women are about 6.4 percentage points less likely to be recidivist. Those individuals in state prisons are about 7 percentage points more likely to be recidivists. This is likely due to the fact that the average individual is less likely to reguarly commit federal crimes, so those in federal prison are more often first offenders. 
