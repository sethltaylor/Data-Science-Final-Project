...
# @ seth: Code works now! You extracted the file into getwd(), but then to attempted reading 'temp' which remained a Zip folder

## Jeff's Code to extract zip files from Github ##

# Federal Numeric Data
...
fed.num <- read.csv(temp, header = TRUE)

# State Numeric Data
url <- "https://github.com/GeorgetownMcCourt/Predicting-Recidivism/raw/master/Data/StateData.zip" #url + file name goes here
temp = tempfile() #Create temp file
download.file(url, temp) ##download the URL direct to the temp file
temp <- unzip(temp, exdir=getwd()) ##unzip that file
