...
for(i in 1:length(predict.glm.val)){
  if(is.na(predict.glm.val[i]) == FALSE){
    val.history[i] <- trainset$CH_CRIMHIST_COLLAPSED[i]
  } 
}
...
meanf1(complete.cases(train.history), complete.cases(train.recid)) #I think this is correct

meanf1(complete.cases(trainset$CH_CRIMHIST_COLLAPSED), complete.cases(train.recid)) #I think this shifts the rows incorrectly because it deletes more cases in train.recid than in our training set
...
meanf1(complete.cases(val.history), complete.cases(val.recid)) #I think this is correct

meanf1(complete.cases(valset$CH_CRIMHIST_COLLAPSED), complete.cases(val.recid)) #I think this is wrong for the same reason as above
