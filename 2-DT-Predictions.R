...
model.errors[7,c(2:4)] <- c(mean(error.train), mean(error.test), mean(error.val)) # low mean F1: ~65%
...
model.errors[7,5] <- meanf1(full.d$CH_CRIMHIST_COLLAPSED, pred.full)
...
# add mean error values to model.errors
model.errors[9,c(2:4)] <- c(mean(error.train), mean(error.test), mean(error.val)) # mean F1 hardly better: ~66%
