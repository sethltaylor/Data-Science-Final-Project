...
plot_ly(x = graphs$Drug.rec, type = "histogram", name = 'Drug Crimes') %>%
...
meanf1(full.opt$CH_CRIMHIST_COLLAPSED, pred.full.opt)
...
ggplot(roc.knn, aes(x=ROC, colour = ROC, fill=ROC)) + geom_histogram(stat="count") + labs(title="KNN Predictions")
```
...
meanf1(full.opt$CH_CRIMHIST_COLLAPSED, pred.full.dt)
...
ggplot(roc.dt, aes(x=ROC, colour = ROC, fill=ROC)) + geom_histogram(stat="count")  + labs(title="Decision Tree Predictions")
...
meanf1(full.opt$CH_CRIMHIST_COLLAPSED, pred.full.rf)
...
ggplot(roc.rf, aes(x=ROC, colour = ROC, fill=ROC)) + geom_histogram(stat="count") + labs(title="Random Forest Predictions")
