...
model.errors[12,c(2:4)] <- c(mean(error.train), mean(error.test), mean(error.val)) # mean F1: ~68%
...
model.errors[12,5] <- meanf1(full.r$CH_CRIMHIST_COLLAPSED, pred.full)
...
# add mean error values to model.errors
model.errors[13,c(2:4)] <- c(mean(error.train), mean(error.test), mean(error.val)) 
# mean F1 higher for validate but lower for test

# rerun Model on full dataset
rf.full.16 <- randomForest(as.factor(CH_CRIMHIST_COLLAPSED) ~ OFFENSE_VIOLENT + OFFENSE_DRUG + SES_PHYSABUSED_EVER + 
                          SES_PARENTS_INCARCERATED + SES_HASCHILDREN + EDUCATION + AGE_CAT +
                          black.nh + hispanic + asian + SES_SEXABUSED_EVER + SES_FAMILY_INCARCERATED + 
                          DRUG_ANYREG + DRUG_HROPTME + DRUG_METHATME + LIFE_SENTENCE,
                          data = full.r)
pred.full <- predict(rf.full.16, full.r, type = "class")
model.errors[13,5] <- meanf1(full.r$CH_CRIMHIST_COLLAPSED, pred.full) # mean f1 better with RF 18 vars default
...
# add mean error values to model.errors
model.errors[14,c(2:4)] <- c(mean(error.train), mean(error.test), mean(error.val))
...
rf.full <- randomForest(as.factor(CH_CRIMHIST_COLLAPSED) ~ OFFENSE_VIOLENT + OFFENSE_DRUG + SES_PHYSABUSED_EVER + 
                          ...
                          data = full.r, mtry=4)
pred.full <- predict(rf.full, full.r, type = "class")
model.errors[14,5] <- meanf1(full.r$CH_CRIMHIST_COLLAPSED, pred.full) #mean f1 for full dataset lower than with RF default
...
# add mean error values to model.errors
model.errors[15,c(2:4)] <- c(mean(error.train), mean(error.test), mean(error.val))
# only slightly better than RF default for train and test
...
pred.full <- predict(rf.full.opt.opt, full.opt, type = "class")
model.errors[15,5] <- meanf1(full.opt$CH_CRIMHIST_COLLAPSED, pred.full) #mean f1 for full dataset lower than with RF default

# confusion matrix
table(full.opt$CH_CRIMHIST_COLLAPSED, pred.full) # sensitivity and specificity pretty much like RF default
